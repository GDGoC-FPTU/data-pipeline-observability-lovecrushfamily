...
def extract(file_path):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        print(f"Extract complete. {len(data)} records found.")
        return data
...
def validate(data):
    ...
    print(
        "Validation summary: "
        f"{len(valid_records)} valid records kept, {error_count} invalid records dropped."
    )
...
def transform(data):
    ...
    if df.empty:
        df['discounted_price'] = pd.Series(dtype='float64')
        df['processed_at'] = pd.Series(dtype='object')
        return df
